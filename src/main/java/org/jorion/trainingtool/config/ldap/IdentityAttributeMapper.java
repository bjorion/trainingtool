...
public class IdentityAttributeMapper implements AttributesMapper<GroupInfraIdentityDTO> {
     ...
     */
    private static boolean isManager(Attributes attribs) {

        boolean result = false;
        Attribute attrib = attribs.get(LdapConstants.DIRECT_REPORTS);
     ...
     */
    protected static String readAttribute(Attributes attribs, String name) {

        if (attribs.get(name) == null) {
            return "";
        }

        String attr = attribs.get(name).toString();
