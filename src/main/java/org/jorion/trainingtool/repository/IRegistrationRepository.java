...
public interface IRegistrationRepository extends JpaRepository<Registration, Long> {
    ...
    Optional<Registration> findRegistrationByIdAndManager(@Param("id") Long id, @Param("managerName") String managerName);

    @Query(SQL_REGS_BY_STATUSES)
    List<Registration> findRegistrationsByStatuses(@Param("statuses") EnumSet<RegistrationStatus> statuses);

    @Query(SQL_REGS_BY_STATUSES_AND_MANAGERNAME)
    List<Registration> findRegistrationsByStatusesAndManager(@Param("statuses") EnumSet<RegistrationStatus> statuses, @Param("managerName") String managerName);

    @Query(SQL_REGS_BY_EXAMPLE_AND_MANAGERNAME)
    List<Registration> findAllByExample(@Param("lastName") String lastName, @Param("startDate") LocalDate startDate, @Param("status") RegistrationStatus status,
